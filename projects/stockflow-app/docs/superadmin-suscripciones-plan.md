...
**Decisiones del owner tomadas antes de escribir esto:** cobro por **transferencia a
alias** (sin integración de pagos) · el panel **se queda en `/super`**, se endurece, no se
muda.
...
| Momento | Qué pasa | Canal |
|---|---|---|
| Día 10 | Recordatorio suave | **Email al dueño** + fila `notifications` |
| Día 17 | Escalada: *"se suspende el 25"* | Email + aviso más visible |
| Día 25 | Suspensión vía `stores.status` | Guarda existente (`session.ts:93`, `049:175-177`) |
...
(`vercel.json:1-16`) — **no hay ninguno de día 10**.

---

## F · Login para tres actores

> ⚠️ **Existe un reference-lock APROBADO** — `docs/reference-locks/marca-login-shell.md`,
> con OK del owner sobre el prototipo vivo (2026-07-22). Es **autoridad nivel 4** de
> `design-layer-plan.md` y *"manda en su sección"*. **Rediseñar el login exige un OK nuevo
> del owner**, y el lock se reescribe después. No es una decisión que pueda tomar el
> equipo técnico.
...
**Pendiente del owner:** confirmar su condición fiscal (monotributo / responsable
inscripto), que define el comprobante exacto.
...
1. **¿Cuál es el precio mensual?** Figura como decisión pendiente en `roadmap.md:21`. Sin
   él no hay MRR, ni escalera, ni panel de deuda.
2. **¿Qué día vence** — fecha fija para todos, o aniversario del alta de cada negocio?
3. **¿Tu condición fiscal?** Define el comprobante.
4. **¿El primer cliente paga desde el mes 1**, o hay período de gracia?
5. **¿Autorizás rediseñar el login**, sabiendo que rompe un lock aprobado por vos?
