...
import { ArrowLeft, Ticket, Clock3, CheckCircle2, AlertCircle, Wallet } from "lucide-react";
import { createSupabaseServer } from "@/lib/supabase/server";
import { createAdminClient } from "@/lib/supabase/admin";
import { startCheckout } from "./actions";
import { BuyButton } from "./buy-button";

export const metadata = { title: "Comprar pack" };
  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-2xl px-5 pb-16 pt-8 lg:px-8">
      ...
      <header className="mt-3">
        <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Comprar pack</h1>
      ...
      {status === "ok" ? (
        <p className="mt-5 flex items-center gap-2 rounded-xl border border-success/30 bg-success/10 px-3 py-2 text-sm text-success">
          ...
          Recibimos tu pago. Apenas se acredite vas a ver el pack en tu saldo.
        </p>
      ) : null}
      {status === "pending" ? (
      ...
      {!mpConnected ? (
        <div className="mt-6 rounded-2xl border border-dashed border-border bg-card/60 px-6 py-12 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">
            Este estudio todavía no tiene el pago online activo. Consultá en el mostrador para comprar tu pack.
          </p>
        </div>
      ) : passes.length > 0 ? (
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-border bg-card/60 px-6 py-12 text-center">
          ...
          <p className="mt-3 text-sm text-muted-foreground">El estudio todavía no publicó packs.</p>
        </div>
      )}
